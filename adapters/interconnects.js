...
async function enrichArticleText(article) {
  ...
  try {
    ...
    console.warn(`[interconnects] Artikeltext konnte nicht geladen werden (${article.titel}): ${err.message}`);
  }
  return article;
}
...
function parseRss(xml) {
  ...
  while ((match = itemRegex.exec(xml)) !== null) {
    ...
    articles.push({ titel, url, datum, quelle: 'interconnects', rohtext });
  }
