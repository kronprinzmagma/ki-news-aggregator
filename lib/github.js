 ...
 */
export async function uploadReleaseAsset(token, release, name, buffer, contentType = 'application/octet-stream') {
  const existing = (release.assets || []).find((a) => a.name === name);
  if (existing) {
    // Fehler beim Delete dürfen den Prozess nicht killen (Unhandled Rejection) –
    // wir versuchen den Upload trotzdem; GitHub lehnt Namens-Duplikate dann mit
    // HTTP 422 ab, was unten als normaler Fehlschlag (null) behandelt wird.
    try {
      await deleteReleaseAsset(token, existing.id);
    } catch (err) {
      console.warn(`Asset-Delete fehlgeschlagen (${err.message}) – versuche Upload trotzdem.`);
    }
  }
