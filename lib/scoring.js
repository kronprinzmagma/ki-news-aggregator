...
- "The User Is Visibly Frustrated" — reflektierter Praxis-Essay eines glaubwürdigen Builders über die UX der Agenten-Arbeit; hoher Lernwert auch ohne API-Details
- "llm-anthropic 0.25.1" — Plugin-Release, das ein NEUES Claude-Modell für eigene Tools erschliesst; direkt nutzbar für jemanden, der mit Claude baut

Wichtige Unterscheidungen (häufige Fehlerquellen):
- **Dev-Tool ohne KI-Kern**: Releases von Datenbank-Tools, Sync-Bibliotheken, Editor-/Infrastruktur-Tools OHNE LLM-/KI-Bezug im Kern (z.B. "datasette 1.0a31", eine CRDT-Bibliothek) → maximal Score 2, egal wie konkret die API-Details sind. Konkretheit ersetzt keinen KI-Bezug.
- **Plugin-Release-Regel mit Ausnahme**: Bugfix-/Minor-Releases derselben Plugin-Familie ohne neue Capability ("datasette-referrer-policy 0.1", "llm-gemini 0.32a0") → Score 1–3. ABER: Erschliesst ein Release Zugang zu einem neuen Modell oder einer neuen Modell-Capability (z.B. llm-anthropic mit Support für ein frisches Claude-Release), ist das für die Persona direkt wertvoll → Score 4–5.
- **Praxis-Essays zählen**: Reflektierte Erfahrungsberichte glaubwürdiger Builder über das Arbeiten mit KI-Agenten (Workflow, UX, Kosten-Nutzen, Psychologie der Agent-Nutzung) sind KEINE "persönlichen Experimente ohne Erkenntnis" – sie haben hohen Lernwert für die Persona → Score 4–5, auch ohne Code/API-Details.
- **Capability-Evidenz schlägt Trend-Watch-Cap**: Empirische Studien/Benchmarks mit klarem, messbarem Ergebnis (z.B. Modell übertrifft Fachexperten in Domäne X) sind starke strategische Signale → Score 4–5. Die Max-3-Regel für Trend-Watch gilt nur für vage Artikel ohne messbares Ergebnis.
- Persönliche Experimente und Quoting-Posts ohne eigene Erkenntnis bleiben Score 2–3
...
Wichtig: Ein technischer Artikel darf Score 4–5 erreichen, wenn er gut erklärt und für einen PM ohne reinen Dev-Background nutzbar ist. Score 5 ist aber kein Freifahrtschein für Infrastruktur-Tieftaucher ohne Produktbezug. Reine Trend-Watch-Artikel (kein Code, keine API, keine Adoption, kein messbares Studienergebnis) sind maximal Score 3.
