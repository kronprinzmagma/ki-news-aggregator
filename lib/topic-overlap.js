import { TOPIC_STOPWORDS, LAB_QUELLEN } from './config.js';

// Kurze Fach-Tokens, die trotz Mindestlänge immer durchkommen sollen –
// Modell- und Tooling-Kürzel sind oft die stärksten Themen-Signale.
const SHORT_TOKEN_WHITELIST = new Set([
  'gpt', 'mcp', 'api', 'llm', 'gpu', 'tts', 'rag', 'sdk',
  'o1', 'o3', '4o', 'ki', 'ai',
]);

export function tokenize(text) {
  // Unicode-aware: \W+ ohne u-Flag matcht keine Umlaute → "kündigt" zerfiel
  // bisher in ["k", "ndigt"]. Mit \p{L}\p{N} bleiben Wörter mit ä/ö/ü/ß intakt.
  // Mindestlänge 3 (statt > 3), damit Kürzel wie "gpt"/"mcp" nicht rausfallen;
  // generische Kurzwörter fängt die Stopword-Liste ab.
  return new Set(
    (text || '').toLowerCase().split(/[^\p{L}\p{N}]+/u).filter(
      w => (w.length >= 3 || SHORT_TOKEN_WHITELIST.has(w)) && !TOPIC_STOPWORDS.has(w)
    )
  );
}
 ...
 * (Tie-Break: Lab-Quellen vor anderen, wie in deliver.js – deterministisch).
 * Der schwächere von zwei Artikeln mit >= threshold gemeinsamen Tokens
 * erhält penalty (Default -1, min. 1); pro Artikel maximal eine Penalty.
 * Mutiert das Array nicht.
 */
export function applyEventDedup(scored, { threshold = 3, penalty = 1, onPenalty } = {}) {
  ...
  for (let i = 0; i < adjusted.length; i++) {
    for (let j = i + 1; j < adjusted.length; j++) {
      if (adjusted[j].dedup_penalty) continue; // max. eine Penalty pro Artikel
