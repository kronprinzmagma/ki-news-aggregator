...
function retryDelay(retries) {
  // Exponentieller Backoff mit Jitter, damit parallele Worker nicht im
  // Gleichtakt retrien (Thundering Herd nach 429/Overload).
  return RETRY_DELAY_MS * 2 ** retries + Math.random() * 1000;
}

/** JSON.parse mit Kontext-Fehlermeldung statt rohem SyntaxError. */
function parseJsonResponse(body, status, context) {
  try {
    return JSON.parse(body);
  } catch {
    throw new Error(`${context}: Ungültiges JSON (HTTP ${status}) – ${String(body).slice(0, 200)}`);
  }
}
...
export async function callClaude({
  ...
  for (let retries = 0; ; retries++) {
    ...
    const parsed = parseJsonResponse(body, status, 'Claude API Response');
...
export async function claudeBatch(requests, {
  ...
  const submitted = parseJsonResponse(submitRes.body, submitRes.status, 'Batch-Submit-Response');
  ...
  while (batch.processing_status !== 'ended') {
    ...
    try {
      ...
      console.warn(`[${logTag}] Poll-Antwort kein valides JSON, retry …`);
      continue;
    }
