...
export class BatchStuckError extends Error {
  constructor(batchId, elapsedMs) {
    super(`Batch ${batchId} hängt: 0 Requests verarbeitet nach ${Math.round(elapsedMs / 60000)}min – Fallback auf Sync-Modus`);
...
export async function claudeBatch(requests, {
  ...
  while (batch.processing_status !== 'ended') {
    ...
    } else if (completedCount === 0 && Date.now() - lastProgressAt > stuckTimeoutMs) {
      await cancelBatch(batchId, logTag);
      throw new BatchStuckError(batchId, Date.now() - startedAt);
    }
  }
